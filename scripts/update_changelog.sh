...
GITHUB_USER="CosmWasm"
GITHUB_PROJECT="cw-storage-plus"
...
github_changelog_generator -u "$GITHUB_USER" -p "$GITHUB_PROJECT" --base CHANGELOG.md $ORIGINAL_OPTS || cp /tmp/CHANGELOG.md.$$ CHANGELOG.md

if [ -n "$UPCOMING_TAG" ]
then
  # Add "upcoming" version tag
  TODAY=$(date "+%Y-%m-%d")
  sed -i "s+\[Full Changelog\](https://github.com/$GITHUB_USER/$GITHUB_PROJECT/compare/\(.*\)\.\.\.HEAD)+[Full Changelog](https://github.com/$GTHUB_USER/$GTHUB_PROJECT/compare/$UPCOMING_TAG...HEAD)\n\n## [$UPCOMING_TAG](https://github.com/$GITHUB_USER/$GITHUB_PROJECT/tree/$UPCOMING_TAG) ($TODAY)\n\n[Full Changelog](https://github.com/$GITHUB_USER/$GITHUB_PROJECT/compare/\1...$UPCOMING_TAG)+" CHANGELOG.md
fi
